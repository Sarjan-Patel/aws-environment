...
async function updateLiveUtilization(
  ...
  // Update RDS instance live metrics
  ...
  if (rdsInstances?.length) {
    for (const rds of rdsInstances) {
      ...
      if (rds.env === 'prod') {
        ...
        connections = randomInt(20, 100)
      } else {
        cpu = weekend ? randomBetween(2, 10) : randomBetween(5, 25)
        connections = weekend ? randomInt(0, 5) : randomInt(5, 30)
      }
  ...
  // Update cache cluster live metrics
  ...
  if (cacheClusters?.length) {
    for (const cache of cacheClusters) {
      ...
      if (cache.env === 'prod') {
        cpu = randomBetween(20, 50)
        connections = randomInt(50, 200)
      } else {
        cpu = weekend ? randomBetween(1, 5) : randomBetween(5, 20)
        connections = weekend ? randomInt(0, 5) : randomInt(5, 30)
      }
  ...
  // Update load balancer live metrics
  ...
  if (loadBalancers?.length) {
    for (const lb of loadBalancers) {
      ...
      if (lb.env === 'prod') {
        requestRate = randomBetween(100, 5000) // 100-5000 requests/minute
      } else {
        requestRate = weekend ? randomBetween(0, 10) : randomBetween(10, 500)
      }

      await supabase
        .from('load_balancers')
        .update({
          current_request_rate: Math.round(requestRate * 100) / 100,
          updated_at: new Date().toISOString(),
        })
        .eq('id', lb.id)
    }
  ...
  // Update Lambda function metrics (memory usage for over-provisioning detection)
  const { data: lambdas } = await supabase
    .from('lambda_functions')
    .select('id, memory_mb, env')
    .eq('account_id', accountId)

  if (lambdas?.length) {
    for (const lambda of lambdas) {
      // Simulate memory usage: typically 20-60% of allocated memory
      // Prod functions tend to use more memory than dev/staging
      let memoryUtilization: number
      if (lambda.env === 'prod') {
        memoryUtilization = randomBetween(0.4, 0.7) // 40-70% utilization
      } else {
        memoryUtilization = randomBetween(0.15, 0.45) // 15-45% utilization (over-provisioned)
      }

      const avgMemoryUsed = Math.round(lambda.memory_mb * memoryUtilization)

      await supabase
        .from('lambda_functions')
        .update({
          avg_memory_used_mb_7d: avgMemoryUsed,
          updated_at: new Date().toISOString(),
        })
        .eq('id', lambda.id)
    }
...
async function createIdleRdsInstance(
  ...
  const currentCpu = randomBetween(2, 15) // Very low CPU = idle
  const currentConnections = randomInt(1, 10) // Very few connections
  ...
  const { data: rds, error } = await supabase
    ...
    .insert({
      ...
      avg_cpu_7d: currentCpu,
      avg_connections_7d: currentConnections,
      current_cpu: currentCpu, // Real-time metric for monitoring dashboard
      current_connections: currentConnections, // Real-time metric for monitoring dashboard
    })
    .select('id')
    .single()
...
async function createIdleLoadBalancer(
  ...
  const avgRequestCount = randomBetween(0, 100) // Almost no traffic
  const currentRequestRate = randomBetween(0, 10) // Requests per minute
  ...
  const { data: lb, error } = await supabase
    ...
    .insert({
      ...
      region: region,
      hourly_cost: hourlyCost,
      avg_request_count_7d: avgRequestCount,
      current_request_rate: currentRequestRate, // Real-time metric for monitoring dashboard
    })
    .select('id')
    .single()
...
async function createOverProvisionedLambda(
  ...
  const { data: lambda, error } = await supabase
    ...
    .insert({
      ...
      avg_memory_used_mb_7d: actualMemoryUsed, // For over-provisioning detection
    })
    .select('id')
    .single()
...
async function createIdleCacheCluster(
  ...
  const currentCpu = randomBetween(1, 10) // Very low
  const currentConnections = randomInt(0, 5) // Almost no connections
